//
// Created by How Yu on 2022/4/27.
//
...
int main(int argc, char **argv) {
    ...
    int np = atoi(argv[1]);   // Holds the number of particles
    ...
    for (i = 0; i < np; i++) {
        for (dim = 0; dim < nd; dim++) {
            if (pos[dim+i*nd] < 0) { // Particle has exited the lower boundary.
                pos[dim+i*nd] = std::fmod(pos[dim+i*nd], L)+L;
            } else if (pos[dim+i*nd] > L) { // Particle has exited the upper boundary.
                pos[dim+i*nd] = std::fmod(pos[dim+i*nd], L);
...
//#pragma acc data copy(acc[0:np*nd],vel[0:np*nd],pos[0:np*nd])
    for (i = 0; i < np; i++) {
...
//#pragma acc data create(pos[0:np*nd], acc[0:np*nd], vel[0:np*nd])

#pragma acc data copy(acc[0:np*nd], vel[0:np*nd],pos[0:np*nd])
#pragma acc parallel loop
    for(i=0;i<np;i++){

#pragma acc loop reduction(+:Epot,Ekin) private(r2,finst,dim,dist_diff,r6_over,fp)
        for (j = 0; j < np; j++) {
            if(j==i){continue;}
            else{r2 = 0.0;
                in_range = true;
