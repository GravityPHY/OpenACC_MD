...
void initCellReference(int nd, int L, vector<int> *cell_reference);
void print_2D_cell_refe(int nd, int L, int N_cell, vector<int> *cell_reference);
...
int main(int argc, char **argv) {
    ...
    int *cell;                // Holds the cell index of the particles
    vector<int> *cell_reference; // Holds the static list of index of adjacent cells
    int N_cell = (int)pow(2.0*L / R_cut, nd); // Number of cells
    ...
    cell = new int[np];
    cell_reference = new vector<int>[N_cell]; // L / (R_cut/2) ^ nd
...
void initCellReference(int nd, int L, vector<int> *cell_reference){
    int Z = 2*L / R_cut;
    int i, j, k; // index of one cell
    int a, b, c; // offset of the index i, j
    int m, n, p; // real index of neighbor of one cell

    if(nd == 2){
        for (i=0; i < Z; i++){
            for (j = 0; j<Z; j++){
                // now for each cell 
                for (a = -2; a < 3; a++){ // offset of i
                    for (b = -2; b < 3; b++){ // offset of j
                        m = (i+a+L) % L;
                        n = (j+b+L) % L;
                        cell_reference[i + Z*j].push_back(m*Z + n);
                    }
                }
            }
        ...
        for(i=0; i<Z; i++){
            for (j=0; j<Z; j++){
                for(k=0; k<Z; k++){
                    ...
                    for (a = -2; a < 3; a++){ // offset of i
                        for (b = -2; b<3; b++){ // offset of j
                            for (c = -2; c<3; c++){ // offset fo k
                                m = (i+a+L) % L;
                                n = (j+b+L) % L;
                                p = (k+c+L) % L;
                                cell_reference[i + Z*j + Z*Z*k].push_back(m*Z*Z + n*Z + p);
                            }
                        }
                    }
                }
...
void print_2D_cell_refe(int nd, int L, int N_cell, vector<int> *cell_reference){
    int i; // index of one cell

    for (i = 0; i<N_cell; i++){
        vector<int> ptr = cell_reference[i];
        printf("[%d]: size: %d \t", i, ptr.size());
        for (vector<int>::iterator j = ptr.begin(); j!= ptr.end(); j++){
            printf("%d -> ", *j);
        }
        printf("\n");
    }
